#!/usr/bin/env python3
# -*- coding: utf-8 -*-
from pathlib import Path
import argparse, json, html, re, unicodedata
ROOT=Path(__file__).resolve().parent
BOOKS=ROOT/'books'
TEMPLATE=ROOT/'template.html'
OUT=ROOT/'index.html'
TR={'а':'a','б':'b','в':'v','г':'h','ґ':'g','д':'d','е':'e','є':'ie','ж':'zh','з':'z','и':'y','і':'i','ї':'i','й':'i','к':'k','л':'l','м':'m','н':'n','о':'o','п':'p','р':'r','с':'s','т':'t','у':'u','ф':'f','х':'kh','ц':'ts','ч':'ch','ш':'sh','щ':'shch','ь':'','ю':'iu','я':'ia','ы':'y','э':'e','ъ':'','ё':'yo'}
def slug(s):
    s=''.join(TR.get(c,c) for c in s.lower())
    s=unicodedata.normalize('NFKD',s)
    s=''.join(c for c in s if not unicodedata.combining(c))
    return re.sub(r'[^a-z0-9]+','-',s).strip('-') or 'book'
def scan():
    BOOKS.mkdir(exist_ok=True)
    pdfs=list(BOOKS.glob('*.pdf'))+list(BOOKS.glob('*.PDF'))
    used=set(); arr=[]
    for p in sorted(pdfs,key=lambda x:x.name.lower()):
        title=p.stem.strip(); base=slug(title); ident=base; n=2
        while ident in used:
            ident=f'{base}-{n}'; n+=1
        used.add(ident)
        arr.append({'id':ident,'title':title,'file':'books/'+p.name,'category':'PDF','tags':[]})
    return arr
if __name__=='__main__':
    ap=argparse.ArgumentParser(); ap.add_argument('--title',default='Моя PDF-бібліотека'); a=ap.parse_args()
    data=json.dumps(scan(),ensure_ascii=False,separators=(',',':')).replace('</','<\\/')
    html_text=TEMPLATE.read_text(encoding='utf-8').replace('__TITLE__',html.escape(a.title)).replace('__BOOKS__',data)
    OUT.write_text(html_text,encoding='utf-8')
    print('Generated index.html')
