...
def scan_books():
    ...
    for pdf in sorted(pdfs,key=lambda p:p.name.lower()):
        title=pdf.stem.strip(); base=slug(title); ident=base; n=2
        while ident in used:
            ident=f'{base}-{n}'; n+=1
        used.add(ident)
        books.append({'id':ident,'title':title,'file':'books/'+pdf.name,'category':'PDF','tags':[]})
    return books
def main():
    ...
    text=TEMPLATE.read_text(encoding='utf-8').replace('__TITLE__',html.escape(args.title)).replace('__BOOKS__',data)
